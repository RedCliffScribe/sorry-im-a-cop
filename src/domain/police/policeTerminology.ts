const rankLabels: Array<[RegExp, string]> = [
  ...
  [/^Probationary Inspector(?:\s*[（(]见习督察[）)])?$/i, '见习督察'],
  [/^Senior Inspector(?:\s*[（(]高级督察[）)])?$/i, '高级督察'],
  [/^Chief Inspector(?:\s*[（(]总督察[）)])?$/i, '总督察'],
  [/^Inspector(?:\s*[（(]督察[）)])?$/i, '督察'],
  [/^Senior Superintendent(?:\s*[（(]高级警司[）)])?$/i, '高级警司'],
  [/^Superintendent(?:\s*[（(]警司[）)])?$/i, '警司'],
  [/^Assistant Commissioner(?:\s*[（(]助理处长[）)])?$/i, '助理处长'],
